...
const SCATTER_VECTORS = [
  { x: -43, y: -24, r: -72, s: 0.62, a: 0.76 },
  { x: -28, y: 27, r: 41, s: 0.48, a: 0.58 },
  { x: 34, y: -31, r: 67, s: 0.74, a: 0.82 },
  { x: 46, y: 17, r: -38, s: 0.52, a: 0.68 },
  { x: -48, y: 8, r: 29, s: 0.72, a: 0.9 },
  { x: 18, y: 34, r: -63, s: 0.58, a: 0.68 },
  { x: -12, y: -35, r: 22, s: 0.44, a: 0.52 },
  { x: 49, y: -7, r: 48, s: 0.86, a: 0.9 },
  { x: -35, y: 33, r: -51, s: 0.57, a: 0.62 },
  { x: 9, y: -39, r: 83, s: 0.68, a: 0.76 },
  { x: 39, y: 31, r: -26, s: 0.55, a: 0.64 },
  { x: -46, y: -29, r: 58, s: 0.46, a: 0.54 },
  { x: 27, y: 8, r: -94, s: 1.04, a: 0.92 },
  { x: -21, y: -13, r: 103, s: 0.39, a: 0.48 },
  { x: 44, y: -23, r: -18, s: 0.67, a: 0.82 },
  { x: -41, y: 19, r: 76, s: 0.81, a: 0.82 },
  { x: 13, y: 29, r: -112, s: 1.18, a: 0.94 },
  { x: -31, y: -37, r: 37, s: 0.5, a: 0.58 },
  { x: 48, y: 35, r: 71, s: 1.28, a: 0.96 },
  { x: -8, y: 38, r: -39, s: 0.63, a: 0.7 },
] as const;
...
export function HomeServicesShowcase() {
  ...
  useGSAP(
    () => {
      ...
      const aiCard = section.querySelector<HTMLElement>(
        '[data-service-card="ai"]',
      );
      ...
      gsap.set(
        [...aiMotifs, ...websiteMotifs, ...webMotifs, ...wordpressMotifs],
      ...
      words.forEach((word, wordIndex) => {
        ...
        wordChars.forEach((char, charIndex) => {
          const vector =
            SCATTER_VECTORS[(wordIndex * 7 + charIndex) % SCATTER_VECTORS.length];
          const depthFactor = 0.86 + wordIndex * 0.055;
          const staggerOffset = wordIndex * 0.011 + charIndex * 0.0022;

          introTimeline.to(
            char,
            {
              x: `${vector.x * depthFactor}vw`,
              y: `${vector.y * depthFactor}vh`,
              rotation: vector.r,
              scale: vector.s,
              autoAlpha: vector.a,
              duration: 0.18,
      ...
      introTimeline.to(
        wordStack,
        { scale: 0.98, duration: 0.1, ease: "none" },
      ...
      introTimeline.to(
        wordStack,
        { autoAlpha: 0, duration: 0.045, ease: "none" },
        0.465,
      );
      ...
      const progressTrigger = ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          ...
          section.style.setProperty(
            "--services-progress",
            state.master.toFixed(4),
          );
          section.style.setProperty(
            "--services-detail-a",
            state.detailA.toFixed(4),
          );
          section.style.setProperty(
            "--services-detail-b",
            state.detailB.toFixed(4),
          );
          section.style.setProperty(
            "--services-detail-c",
            state.detailC.toFixed(4),
          );
          section.style.setProperty(
            "--services-final",
            state.final.toFixed(4),
          );
          ...
          gsap.set([...aiMotifs, ...websiteMotifs], {
            scale: mix(0.9, 1, state.detailA),
            autoAlpha: state.detailA * 0.68,
          });
          gsap.set(webMotifs, {
            scale: mix(0.9, 1, state.detailB),
            autoAlpha: state.detailB * 0.7,
          });
          gsap.set(wordpressMotifs, {
            scale: mix(0.9, 1, state.detailC),
            autoAlpha: state.detailC * 0.72,
          });
        },
      });
