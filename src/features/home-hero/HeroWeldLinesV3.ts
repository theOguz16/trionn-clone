...
export class HeroWeldLines {
  ...
  setAnchors(anchors: ScreenPoint[]) {
    ...
    for (let index = 0; index < 3; index += 1) {
      ...
      if (
        ...
        distanceSquared(current, next) > 0.36
      ) {
        /*
         * Slight damping keeps the huge screen-space guides from looking
         * nervous while the 3D symbol performs its idle rotation.
         */
        const mix = this.hasExternalAnchors ? 0.72 : 1;
        current.x += (next.x - current.x) * mix;
        current.y += (next.y - current.y) * mix;
        changed = true;
      }
    }
  ...
  private buildPaths() {
    ...
    /*
     * The reference reads as three huge ellipses seen only through a
     * shallow arc. Their midpoint is pinned to the model anchor so the
     * line remains geometrically continuous while the model occludes the
     * center: line -> behind symbol -> line.
     *
     * Larger radii + smaller spans are deliberate. The previous pass was
     * mathematically elliptical but visually too loop-like.
     */
  ...
  private updateLineProgress() {
    for (let index = 0; index < 3; index += 1) {
      const delay = 0.74 + index * 0.1;
      const progress = clamp01((this.elapsed - delay) / 0.92);
  ...
  private drawSampledPath(samples: ScreenPoint[], progress: number) {
    ...
    this.context.moveTo(samples[startIndex].x, samples[startIndex].y);
  ...
  private drawPathLayer(
    ...
    this.context.strokeStyle = `rgba(145,162,188,${alpha * visible})`;
  ...
  private drawBaseLines(scrollProgress: number) {
    ...
    /*
     * Three quiet layers give the guides depth without turning them into
     * bright foreground graphics.
     */
    this.drawPathLayer(1.8, 0.035, scrollProgress);
    this.drawPathLayer(0.82, 0.09, scrollProgress);
    this.drawPathLayer(0.42, 0.17, scrollProgress);
  ...
  private createBolt(
    ...
    for (let index = 0; index <= segmentCount; index += 1) {
      ...
      const jitter = (Math.random() * 2 - 1) * 7.5 * envelope;
    ...
    const offsets = [-2.2, -1.4, -0.7, 0, 0.7, 1.4, 2.2];
    const colors = [
      0x031f61,
      0x0756e8,
      0x27b1ff,
    ...
    const opacities = [0.12, 0.25, 0.52, 0.92, 0.52, 0.25, 0.12];
    ...
    const maxLife = 0.18 + Math.random() * 0.07;
    this.bolts.push({
      geometries,
      lines,
      materials,
  ...
  private updateBolts(delta: number) {
    const baseOpacities = [0.12, 0.25, 0.52, 0.92, 0.52, 0.25, 0.12];
    let writeIndex = 0;
